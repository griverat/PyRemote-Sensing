...
def load_modis(hdf):
    ...
    lon_hdf = gdal.Open(latlon_sds[1])
    _lon = lon_hdf.ReadAsArray()
    lon_hdf = None
    lat_hdf = gdal.Open(latlon_sds[0])
...
#    df5 = df1.copy()
    ...
    with open("{}3x3pwv_MODIS.txt".format(aeronet_station),"w") as file3x3_end, open('failed_files.txt','w') as log_file:
        for hdf in hdf_files:
            sds = template.format(hdf,num)
            qa_sds = template.format(hdf,11)
            try:
                lat,lon, time = load_modis(hdf)
            except:
                log_file.write('{} failed'.format(hdf))
